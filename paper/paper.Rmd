...
umap_vis <- vis_net(umapgt, cut_off_weight = 1, font_size = 40, width = "100%", height = "100%")
...
settlements_cl <- settlements %>%
  ...
  relocate(Settlement_Name, umap_gmm_manual, .before = AUT_admin_lev) %>% 
  rename(Cluster = umap_gmm_manual) %>%
  as.data.frame()
...
```{r bee_plot}
k_bee_plot <- helper_hists_with_clusters(df = settlements_cl %>% select(-Settlement_Name, -Cluster),
                                         clust = settlements_cl %>% pull(Cluster),
                                         n_col = 3)
